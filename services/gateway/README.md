...
```bash
docker compose -f infra/docker-compose.dev.yml up -d api-gateway
```

- Go gateway in `infra/docker-compose.dev.yml`: `http://localhost:42100`
- Node gateway in `infra/docker-compose.pro.yml`: `http://localhost:42100`

## Use with case63 (k6)

```bash
BASE_URL=http://localhost:42100 \
K6_BASE_URL=http://host.docker.internal:42100 \
