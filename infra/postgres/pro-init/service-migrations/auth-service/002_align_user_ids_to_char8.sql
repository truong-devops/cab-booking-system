...
BEGIN
  ...
  IF users_id_type <> 'bpchar' THEN
    CREATE TEMP TABLE tmp_auth_user_id_map ON COMMIT DROP AS
    SELECT
      id::text AS old_id,
      map_text_to_user8(id::text)::text AS new_id
    FROM users;

    ALTER TABLE users ADD COLUMN IF NOT EXISTS id_8 char(8);
    ALTER TABLE refresh_tokens ADD COLUMN IF NOT EXISTS user_id_8 char(8);

    UPDATE users u
    SET id_8 = m.new_id::char(8)
    FROM tmp_auth_user_id_map m
    WHERE u.id::text = m.old_id;

    UPDATE refresh_tokens t
    SET user_id_8 = m.new_id::char(8)
    FROM tmp_auth_user_id_map m
    ...
    ALTER TABLE refresh_tokens RENAME COLUMN user_id_8 TO user_id;
  END IF;
